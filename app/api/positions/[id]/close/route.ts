...
import { closePosition, getAccount, getPositionById, recordTrade, updateAccount } from '@/lib/db';
...
export async function POST(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const position = getPositionById(Number(params.id), auth.userId);
  if (!position) {
  ...
  closePosition(position.id, { closePrice: exitPrice, closeTime, pnl });
  recordTrade({
    userId: auth.userId,
  ...
  updateAccount(auth.userId, updatedBalance, updatedEquity);
